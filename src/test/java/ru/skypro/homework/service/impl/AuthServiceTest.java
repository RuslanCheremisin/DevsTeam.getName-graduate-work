...
public class AuthServiceTest {
    ...
    @BeforeEach
    public void init() {
        user = new User("Username", "password", "firstname", "lastName", "+79990000000");
        registerReq = new RegisterReq(user.getUsername(),user.getPassword(),user.getFirstName(),user.getLastName(),user.getPhone(),user.getRole());
    ...
    @Test
    public void whenRoleIsNotNullRegisterThrowsDataIntegrityViolationException() {
        Assertions.assertThrows(DataIntegrityViolationException.class, ()->authService.register(registerReq, null));
    }
    ...
    @Test
    public void whenUserDoesntExistLoginReturnsFalse(){
        Assertions.assertFalse(authService.login(user.getUsername(), user.getPassword()));
    }
